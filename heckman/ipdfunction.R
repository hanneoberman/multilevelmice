...
mice.impute.3l.2stage.heckman <- function(y, ry, x, wy=NULL, type, pmm=FALSE, ypmm=NULL, meta_method="reml", pred_std=FALSE,...){
  
  if (pred_std == TRUE){
    #x <- apply(x, MARGIN = 2, FUN = function(X) (if(diff(range(X))==0){ if(sum(X) == 0){0} else{1}} else{(X - min(X))/diff(range(X))}))
