...
patterns <- rbind(
  ...
  mutate(across(everything(), as.factor),
         ...
         text = rep(c(rep(c("1","", "", ""), 3), rep(c("2","", "", ""), 2)),2))
# plot
ggplot(patterns, aes(
  x = name,
  y = rownr,
  ...
  scale_color_manual(values = plot_col, name = "Datapoint") +
  facet_wrap( ~ pat, ncol = 2, strip.position = "bottom") +
  theme_minimal() +
  theme(
    legend.position = "none", 
    panel.grid.major = element_blank()) +
  labs(x = "", y = "Observation (e.g., patient)")
# TODO: add patient nr and cluster
# TODO: add grid.lines = "none"
...
miss_col <- mice:::mdc(1:2) %>% setNames(c("FALSE", "TRUE"))

# x = "popular"
# z = "popteach"
plot_conditional <- function(data, x, z, ...){
  ggplot(data, aes(x = get(x), color = !is.na(get(z)))) +
  geom_density() +
  scale_color_manual(values = miss_col, name = paste("Missing", z)) +
  theme_classic() +
  theme(legend.position = "bottom") +
  labs(x = x)
}
plot_conditional(pop, x = "popular", z = "popteach")
plot_conditional(pop, x = "popteach", z = "popular")
```
...
- We'll use Bayesian linear regression to impute the continuous variables, and logistic regression to impute the binary variable `sex`. We do not use the observation identifier `pupil` or cluster identifier `class` as predictors to impute other variables.

```{r pop-ignore}
# dry run to get imputation parameters
ini <- mice(pop, maxit = 0)

# extract imputation methods and adjust
(meth <- ini$meth)
meth[c("extrav", "texp", "popular", "popteach")] <- "norm"
...
imp_ignore <- mice(pop, m = 10, meth = meth, pred = pred, print = FALSE)
...
psych::describe(mice::complete(imp_ignore))[, c("n", "mean", "median", "min", "max", "sd")] #note that this is just 1 imputation, not the pooled results

# compare ICCs before and after imputation
data.frame(vars = names(pop[c(6, 7, 5)]), 
           observed = c(multilevel::ICC1(aov(popular ~ class, pop)), 
                        multilevel::ICC1(aov(popteach ~ class, pop)), 
                        multilevel::ICC1(aov(texp ~ class, pop))), 
           norm     = c(multilevel::ICC1(aov(popular ~ class, complete(imp_ignore))), 
                        multilevel::ICC1(aov(popteach ~ class, complete(imp_ignore))), 
                        multilevel::ICC1(aov(texp ~ class, complete(imp_ignore)))))

# further inspection of the imputations
densityplot(imp_ignore)
```

- As the ICCs show, 100% of the variance in `texp` can be attributed to the clustering variable `class`. This tells us that the multilevel structure of the data should be taken into account. If we don't, we'll end up with incorrect imputations, biasing the effect of the clusters towards zero.
...
imp_predictor <- mice(pop, m = 10, meth = meth, pred = pred, print = FALSE)
...
data.frame(vars = names(pop[c(6, 7, 5)]), 
           observed = c(multilevel::ICC1(aov(popular ~ class, pop)), 
                        multilevel::ICC1(aov(popteach ~ class, pop)), 
                        multilevel::ICC1(aov(texp ~ class, pop))), 
           norm     = c(multilevel::ICC1(aov(popular ~ class, complete(imp_predictor))), 
