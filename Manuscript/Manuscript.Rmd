...
# load data 
popmis <- mice::popmis %>% mutate(
  class = factor(school), 
  .keep = "unused")#readRDS("../Data/popNCR2.RDS") %>% 
  #filter(class %in% clusters) %>% 
  #mutate(class = as.factor(class))
popcomp <- foreign::read.spss("../Data/popular2.sav", to.data.frame = TRUE)
```

In this section we'll go over the different steps involved with imputing incomplete multilevel data. The data we're using is the `popmis` dataset from the `mice` package. This is a simulated dataset with pupils clustered in classes, where the number of units $n = 2000$, and the number of clusters $N = 100$, on 7 variables: 
...
The analysis model corresponding to this dataset is multilevel regression with `popular` as the outcome variable, `sex` and `extrav` as unit-level predictors, and `texp` and `teachpop` as cluster-level predictors. Since the data is simulated and the missingness is induced, we can compare our imputations to the true data generating mechanism. 

The data is created in such a way that the clustering variable `class` explains quite some variance in the outcome variable `popular`. We express this using the intra-class correlation,  ICC $=$ `r round(icc(popular ~ as.factor(class), data = popcomp), 2)`. This tells us that it would be wise to use multilevel modeling. 
...
The missingness is univariate and sporadic, which is illustrated in the missing data pattern in Figure \ref{fig:pop_pat}. The ICC in the incomplete data is `r round(icc(popular ~ as.factor(class), data = na.omit(popmis)), 2)`. 

To develop the best imputation model for the incomplete variable `popular`, we need to know whether the missingness depends on the observed values of other variables. We'll highlight only one other variable to illustrate, but ideally one would inspect all relations. The questions we'll ask are: 'Does the missing data of pupil popularity (`popular`) depend on observed teacher popularity (`teachpop`)?'. This can be evaluated statistically, but visual inspection usually suffices. We'll make a histogram of `teachpop` separately for the pupils with known popularity and missing popularity.

In Figure \ref{fig:pop_dist} we see that **[update this part]** the distribution for the missing `popular` is further to the right than the distribution for observed `popular`. This would indicate a right-tailed MAR missingness. (In fact, this is exactly what happens, because the missingness in these data was created manually.) We've made it observable by examining the relations between the missingness in popular and the observed data in `teachpop`. 
...
# popmis %>% 
#   filter(as.numeric(class) < 5) %>% 
#   ggplot(aes(x = teachpop, color = class)) +
#     geom_density() +
...
plot_conditional(popmis, x = "teachpop", z = "popular", cluster = "class")
...
ICCs <- data.frame(
  ...
  CCA = c(icc(popular ~ class, popmis), 
               icc(teachpop ~ class, popmis),
               icc(texp ~ class, popmis)), 
  ignored = c(icc(popular ~ class, complete(imp_ignored)), 
              icc(teachpop ~ class, complete(imp_ignored)), 
...
ICCs <- ICCs %>% mutate(
           predictor = c(icc(popular ~ class, complete(imp_predictor)), 
                        icc(teachpop ~ class, complete(imp_predictor)), 
...
imp_norm_2l <-
  mice(
    popmis %>% mutate(class = as.integer(class)),
    pred = pred,
    meth = meth,
    maxit = 1,
    print = FALSE
...
ICCs <- ICCs %>% mutate(
           norm = c(icc(popular ~ as.factor(class), complete(imp_norm_2l)), 
                    icc(teachpop ~ as.factor(class), complete(imp_norm_2l)), 
...
ICCs <- ICCs %>% mutate(
           pan = c(icc(popular ~ as.factor(class), complete(imp_pan_2l)), 
                    icc(teachpop ~ as.factor(class), complete(imp_pan_2l)), 
