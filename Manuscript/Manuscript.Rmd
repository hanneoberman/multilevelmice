...
`popNCR` is a simulated dataset with pupils clustered in classes, where the number of cases $n = 2000$, and the number of clusters $N = 100$, on 7 variables: 
...
```{r pop_norm_eval}
# plot(imp_norm)
densityplot(imp_norm, ~popular)
ICCs <- ICCs %>% mutate(
           norm = c(icc(popular ~ as.factor(class), complete(imp_norm)), 
                    icc(popteach ~ as.factor(class), complete(imp_norm)), 
                    icc(texp ~ as.factor(class), complete(imp_norm)))
           )
ICCs
```
...
```{r pop_pan_eval}
# plot(imp_pan)
densityplot(imp_pan, ~popular)
ICCs <- ICCs %>% mutate(
           pan = c(icc(popular ~ as.factor(class), complete(imp_pan)), 
                    icc(popteach ~ as.factor(class), complete(imp_pan)), 
                    icc(texp ~ as.factor(class), complete(imp_pan)))
           )
ICCs
```
