...
We observe that the missing pattern is quite similar across the clusters. However, regarding the weight variable, we notice that is systematically missing in cluster 3.
...
Since the ICC is above 0.1 and as the main analyis will be consider a mixed model, we decide to use two-level (2l) imputation methods. In this imputation process, we include all predictor variables in the main model and since BMI is a composite variable. We also incorporate weight and height variables.
...
```{r obsmar_met}
meth_mar <- meth_mar <- find.defaultMethod(Obesity, ind.clust=1, I.small = 7, ni.small = 100, prop.small = 0.4)
meth_mar["BMI"]<- "~ I(Weight / (Height)^2)"
meth_mar["Age"]<-"2l.2stage.pmm" 
```
For these imputation models, it is necessary to specify the prediction matrix, with the cluster variable labeled as -2 and the predictor variable labeled as 2, encompassing all variables. We need to suprime the variable Time as this variable is not especified in the main model and modify also in the prediction matrix the relationship between BMI, weight and height to avoid circular predictions. Then we proced to run the imputation model.

```{r obsmar_pred,echo = T}
pred_mar <- mice(Obesity, maxit = 0)$pred   # predictor matrix
...
```
```{r obsmar_mice,echo = T,results="hide",include=FALSE}
imp_mar <- mice(data = Obesity, meth = meth_mar, pred = pred_mar, m=10, seed = 123)
```
...
```{r obsmarp_mice,results="hide",include=FALSE}
meth_mar["Weight"]<-"2l.2stage.pmm" 
imp_mar_pmm <- mice(data = Obesity, meth = meth_mar, pred = pred_mar, m=10, seed = 123)
```
```{r obsmarp_plot}
...
Then we proceed to run the imputation model as before 
Impute the missingness:
```{r obsmnar_mice,results="hide",include=FALSE}
imp_mnar <- mice(
  Obesity, # dataset with missing values
  m = 10, # number of imputations
  maxit = 10,
  meth = meth_mnar, #imputation method vector
  pred = pred_mnar, #imputation predictors matrix
  meta_method = "reml",
  pred_std=FALSE,
  printFlag = FALSE
)
...
We then run the imputation model but this time using the option of pmm, to assure that weight values are in the range of the observable data, this can be implemented by seeting the pmm parameter as true.
```{r obsmnarp_mice,results="hide",include=FALSE}
imp_mnar_pmm <- mice(
  Obesity, # dataset with missing values
  m = 10, # number of imputations
  maxit = 10,
...
```{r obsmnarp_plot}
summary(complete(imp_mnar_pmm,"long"))
plot(imp_mnar_pmm)
```
...
plot_models(list_models,mod_name=c("Complete case", "MAR","MAR_pmm", "MNAR","MNAR_S", "MNAR_pmm"))
```
