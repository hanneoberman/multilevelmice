...
meth <- make.method(hiv)
meth["hiv"] <- "2l.heckman"
imp <- mice(hiv, method = meth)
pred <- make.predictorMatrix(hiv)
pred[, "region"] <- -2
pred[, "smoke"] <- -3
imp2 <- mice(hiv, method = meth, predictorMatrix = pred)
```
...
summary(hivdata)
#  We obtain here the random effects for each interviewer, this is an approximation 
#of the interviewer's skill which will be used as an exclusion constraint. 
#Here, since the location of the interviewer was not randomly assigned to the 
#subjects, the assignment was corrected for region and language. 
...
# 
# # MAR imputation model, we used the 2l.binary from the miceadds package
# data_imp_mar <- mice( data= hivdata, # dataset with missing values
...
# # sum(as.numeric(data_imp_mar[["imp"]][["hiv"]][["1"]]))
# # sum(as.numeric(data_imp_heckp[["imp"]][["hiv"]][["1"]]))
# # sum(as.numeric(data_imp_mar[["imp"]][["hiv"]][["2"]]))
# # sum(as.numeric(data_imp_heckp[["imp"]][["hiv"]][["2"]]))
# 
# # Heckman model
...
# meth["hiv"]<-"2l.heckman"
# 
# data_imp_heckp <- mice( hivdata, # dataset with missing values
#                         m = 10,   # number of imputations
