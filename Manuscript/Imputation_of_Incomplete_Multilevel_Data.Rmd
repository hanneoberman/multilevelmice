...
We illustrate how to impute incomplete multilevel data by means of a case study: `impact` from the \pkg{metamisc} package [empirical data on traumatic brain injuries, $n = 11,022$ patients across $N = 15$ studies, @metamisc]. `impact` is traumatic brain injury data with patients, $n = 11022$, clustered in studies, $N = 15$. With the following 11 variables:
...
[TODO: add more info about the complete data.] The analysis model for this dataset is a prediction model with `mort` as the outcome. In this tutorial we'll estimate the adjusted prognostic effect of `ct` on unfortunate outcomes. The estimand is the adjusted odds ratio for `ct`, after including `type`, `age` `motor_score` and `pupil` into the analysis model: 
```{r mod}
...
```{r truth}
fit <- glmer(mod, family = "binomial", data = impact) 
...
```{r cca}
fit <- glmer(mod, family = "binomial", data = na.omit(dat)) 
tidy(fit, conf.int = TRUE, exponentiate = TRUE) 
```
