...
<!-- What does it mean if an estimation model is not able to handle missing data? there are models that implicitaly are able to manage the missingness like the FML where in the likelihood it is used the information of the completed observations and the incomplete ones.  What do you mean with estimation model? the main model you use to analyse the data-->
<!-- AFAIK, when only the outcome is missing, MI and a ML based method lead to similar results. The text appears to suggest that in that case ML based methods are better. what i understood is ML are better in that case but please check it.-->
