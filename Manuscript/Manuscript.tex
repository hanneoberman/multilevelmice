...
variable as `(sample) unit'.\footnote{Alternatives to unit:
  participant/response/record.} For reasons of brevity, we only discuss
clustering between units, not within units (such as in timeseries or
longitudinal data).\footnote{Also, add that we'll only discuss two
  levels, no more?}
...
include: \pkg{mitml}, \pkg{miceadds}, \pkg{mdmb}.\footnote{Rephrase:
  ...
  a software tutorial. We'll keep it practical.} We introduce three case
studies to illustrate the workflow.
