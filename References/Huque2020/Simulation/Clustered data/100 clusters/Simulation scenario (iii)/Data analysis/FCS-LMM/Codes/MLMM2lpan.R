...
for (i in lowerbound:upperbound)
{
  ...
  #dataFCS2$famStruc<-as.factor(dataFCS2$famStruc)
  #dataFCS2$const<-rep(1,nrow(dataFCS2))
  ini2<-mice(dataFCS2,maxit=0)
  pred<-ini2$predictorMatrix
  pred["bmiz",]<-c(-2,1,1,0,2)
  #pred["QoL",]<-c(-2,1,1,2,0)
  meth2<-ini2$method
  meth2[c("bmiz")]<-"2l.pan"
  ...
  write.table(output, "result2lpanRISM.csv",append = TRUE,sep=",",col.names=FALSE)
}
