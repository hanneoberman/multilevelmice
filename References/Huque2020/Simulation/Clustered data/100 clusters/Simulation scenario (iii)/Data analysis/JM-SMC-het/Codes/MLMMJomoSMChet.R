...
for (i in lowerbound:upperbound)
{
  ...
  #substantive model
  formula<-QoL~bmiz+seifa+(1+bmiz|ID)
  imp2<-jomo.lmer(formula,datajomo,level=c(2,2,1,1,1),nimp=30,output=2)
  # Setup imputation model
  impList <- jomo2mitml.list(imp2)
  ## Analysis model 2
  
  imp2.fitMM <- lapply(impList, FUN=function(x){
    lmer(QoL~bmiz+seifa+(1|clus)+(0+bmiz|clus),data=x)
  })
  ...
  pm1<-0
  pm1[(Jomo2l.fit$estimates[2,1]+qt(0.025, Jomo2l.fit$estimates[2,4])*Jomo2l.fit$estimates[2,2]) < -0.20  &  (Jomo2l.fit$estimates[2,1]+qt(0.975, Jomo2l.fit$estimates[1,4])*Jomo2l.fit$estimates[2,2]) > -0.20]<-1
  pm2<-0
  pm2[(Jomo2l.fit$estimates[3,1]+qt(0.025, Jomo2l.fit$estimates[3,4])*Jomo2l.fit$estimates[3,2]) < 0.25 &  (Jomo2l.fit$estimates[3,1]+qt(0.975, Jomo2l.fit$estimates[1,4])*Jomo2l.fit$estimates[3,2]) > 0.25]<-1
  
  output<-as.data.frame(t(c(dataname,beta.Jomo2l,varcom.Jomo2l,se.Jomo2l,pm1=pm1,pm2=pm2,pm3=pm3)))
  write.table(output, "resultJomoSMChet.csv",append = TRUE,sep=",",col.names=FALSE)
}
